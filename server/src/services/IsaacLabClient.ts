...
export interface IsaacLabJob {
  ...
  error?: string;
  /**
   * Honesty label (TASK-184 Phase 3): 'real' when the job ran against the
   * configured Isaac Lab service (ISAAC_LAB_URL), 'mock' when generated by the
   * in-memory mock mode.
   */
  backend: 'mock' | 'real';
  createdAt: Date;
...
export class IsaacLabClient {
  ...
  async submitJob(datasetId: string, config: IsaacLabJobConfig): Promise<IsaacLabJob> {
    ...
    try {
      ...
      return { ...(await response.json() as IsaacLabJob), backend: 'real' };
    } catch (error) {
  ...
  async listJobs(filter?: IsaacLabJobFilter): Promise<IsaacLabJob[]> {
    ...
    try {
      ...
      this.circuitBreaker.recordSuccess();
      const jobs = await response.json() as IsaacLabJob[];
      return jobs.map((j) => ({ ...j, backend: 'real' as const }));
    } catch (error) {
      if (error instanceof CircuitOpenError) throw error;
      this.circuitBreaker.recordFailure();
      throw error;
