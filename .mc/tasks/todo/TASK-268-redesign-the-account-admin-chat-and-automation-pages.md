...
# Redesign the account, admin, chat and automation pages
...
- **Owned directories:** `features/auth`, `app/src/features/settings`, `app/src/features/organizations`, `app/src/features/team`, `app/src/features/a2a`, `app/src/features/processes`, `app/src/src/pages`, `app/src/src/components/demo`, `app/src/src/components/docs` (all under `app/src/`).
- **Routes:** `/login`, `/register`, `/forgot-password`, `/reset-password`, `/set-password`, `/account`, `/settings`, `/organizations`, `/team`, `/a2a`, `/chat`, `/a2a/agents`, `/a2a/agents/:name`, `/a2a/tasks`, `/a2a/events`, `/processes`, `/processes/:id`, `/docs`, the 404 page and `DemoFeaturePlaceholder`.
- **Size and state:** about 13,000 lines of TSX. Organizations and team are the only features with toasts today; processes and chat render `DemoFeaturePlaceholder` in demo mode; the auth pages sit outside the app shell.
