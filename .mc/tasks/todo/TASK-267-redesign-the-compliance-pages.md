...
id: "TASK-267"
aliases: []
title: "Redesign the compliance pages"
slug: "redesign-the-compliance-pages"
status: "todo"
priority: 2
owner: "huhn511"
projects: []
...
- **Owned directories:** `features/compliance`, `app/src/features/oversight`, `app/src/features/approvals`, `app/src/features/explainability`, `app/src/features/gdpr` (all under `app/src/`).
- **Routes:** `/compliance` and every tab it hosts (audit log, oversight, approvals, explainability, GDPR); the redirects `/oversight`, `/approvals`, `/explainability`, `/gdpr` land on those tabs.
- **Size and state:** about 9,500 lines of TSX. Compliance renders `DemoFeaturePlaceholder` in demo mode, so it is tested in live mode. Audit tables are hand-rolled `<table>`s.
