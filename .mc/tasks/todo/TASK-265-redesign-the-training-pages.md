...
id: "TASK-265"
aliases: []
title: "Redesign the training pages"
slug: "redesign-the-training-pages"
status: "in-progress"
priority: 2
owner: "huhn511"
projects: []
...
- **Owned directories:** `features/training`, `app/src/features/datacollection`, `app/src/features/simulation`, `app/src/features/evaluation`, `app/src/features/pipeline` (all under `app/src/`).
- **Routes:** `/pipeline`, `/data-collection`, `/data-collection/new`, `/data-collection/:sessionId`, `/datasets`, `/datasets/:datasetId/episodes`, `/training` (Jobs, Simulation and Evaluation tabs).
- **Size and state:** about 18,000 lines of TSX; `features/training` alone is 11,500. Datasets and Training render `DemoFeaturePlaceholder` in demo mode, so they are tested in live mode. The pipeline breadcrumb (`PipelineBreadcrumb`, `NextStepBanner`) ties these pages together.
