...
# Redesign the robot detail and control center
...
- **Owned directories:** `features/robots` (all under `app/src/`).
- **Routes:** `/robots/:id` (all tabs), `/control-center`, `/robots/:id/cockpit`, and the robots list inside `/fleet?tab=list` (rendered from `features/robots` components).
- **Size and state:** about 11,000 lines of TSX across `components/`, `components/tabs/`, `components/cockpit/`, `components/telemetry/`, `components/visualization/` and `components/voice/`. Heavy glass, raw hues and mono readouts; the cockpit renders `DemoFeaturePlaceholder` in demo mode.
