...
status_note: |
  DONE 2026-08-16 — PR #229. Verified 2026-08-16: routes/runs/findings end-to-end (Prisma + repo/service/scheduler/photo store, /api/patrol/*, robot PatrolRunner/BaselineStore/inspector with capture/inspect blocks, WS fan-out, alerts + compliance events, VDA5050 export, patrol UI redesigned 50cf092); a live "Patrol now" on the house route (day window, no baseline yet) ran 4/4 legs in 2:41 and raised exactly ONE finding — medium "unexpected box in Hallway", en-route semantic, 60 % — i.e. the acceptance shape; promoted to baseline and reloaded (photos resolve to the promoted run). Demo clip `robot-agent/hardware/sim_g1_dds/clips/demo-task212-patrol.mp4` (local, gitignored).
  ...
  POST-MERGE PASS 2026-08-16: two more live runs on the house route (4/4 legs, 2:25-2:49) — baseline/current pairs, per-leg verdicts, promotion, escalation into an incident, and the alert deep link all verified in the browser. Fixed here: a patrol whose camera or checklist model was dead finished "done, 0 findings" (now failed/reasoned and named); promoteRun deleted the baseline photo of a checkpoint whose capture errored; en-route label diffing fired with no baseline at all; a run interrupted by an agent restart stayed running forever (boot sweep closes it) and, server-side, a dropped agent:patrol:finished blocked every future schedule for that robot (reconcileStaleRuns, PATROL_RUN_STALE_MIN); a non-2xx robot answer was recorded as "unreachable"; baseline photos were swept while their pointer lived on; the store let a late event resurrect a finished run; a failed history fetch rendered "No runs yet" and a transient poll failure wiped the route cards; "This is normal" disabled Escalate for good; blind checkpoints looked inspected; finding alerts said the summary twice and showed the raw [finding:...] tag.
---
