...
This Helm chart deploys NeoDEM, a distributed fleet management platform for humanoid robots with EU AI Act compliance features.
...
helm install neodem ./helm/neodem -n neodem --create-namespace
```
...
| `postgres.auth.password` | Database password | `neodem-dev` |
...
helm upgrade neodem ./helm/neodem \
  -f ./helm/neodem/values-production.yaml \
...
helm uninstall neodem -n neodem

# Delete PVCs (WARNING: This deletes all data!)
kubectl delete pvc -n neodem -l app.kubernetes.io/instance=neodem
```
...
```bash
kubectl get pods -n neodem -l app.kubernetes.io/instance=neodem
```
...
```bash
# Server logs
kubectl logs -n neodem -l app.kubernetes.io/component=server

# All component logs
kubectl logs -n neodem -l app.kubernetes.io/instance=neodem --all-containers
```
...
```bash
# Test NATS health
kubectl exec -n neodem deploy/neodem-server -- \
  wget -qO- http://neodem-nats:8222/healthz

# Test database connection
kubectl exec -n neodem deploy/neodem-server -- \
  pg_isready -h neodem-postgres -U neodem
```
