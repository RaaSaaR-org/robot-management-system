...
// Real CPU usage from os.cpus() time deltas. os.loadavg() is ALWAYS [0,0,0]
// on Windows, so a loadavg-based value pins the gauge at a hardcoded-looking
// 0% there. Sampling is rate-limited: telemetry is generated by several
// consumers (100ms fast frames, 2s full frames, REST polls) and a delta over
// a few milliseconds is pure noise, so between samples the last computed
// value is reused. Returns undefined until the first interval has elapsed —
// the UI renders that as 'n/a', never a fake 0.
const CPU_SAMPLE_MIN_INTERVAL_MS = 1000;
let prevCpuTimes: { idle: number; total: number } | null = null;
let prevCpuSampleAt = 0;
let lastCpuUsage: number | undefined;

/** Aggregate idle/total jiffies across all cores, or null when unavailable. */
...
/** Real system CPU usage %, or undefined while no measurement exists yet. */
function getRealCpuUsage(): number | undefined {
  const now = Date.now();
  if (prevCpuTimes && now - prevCpuSampleAt < CPU_SAMPLE_MIN_INTERVAL_MS) {
    return lastCpuUsage;
  }
  const current = readCpuTimes();
  if (!current) return lastCpuUsage;
  const prev = prevCpuTimes;
  prevCpuTimes = current;
  prevCpuSampleAt = now;
  if (!prev || current.total <= prev.total) return lastCpuUsage; // first sample
  const dTotal = current.total - prev.total;
  const dIdle = current.idle - prev.idle;
  lastCpuUsage = Math.min(100, Math.max(0, Math.round(((dTotal - dIdle) / dTotal) * 1000) / 10));
  return lastCpuUsage;
}
