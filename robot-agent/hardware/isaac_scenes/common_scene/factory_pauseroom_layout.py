...
It is USDA (text), not USDC, on purpose: it is 5 kB of reviewable ASCII, it diffs, and it
needs no tooling to read. It is resolved by `base_scene_factory_pauseroom.py` relative to
...
# the README's live-sim section): at 2.5 m the robot needs ~23 s to arrive, and the leaves
# need 1.17 s to open, so the door is always fully open well before the robot is there.
...
def door_advance_openness(current: float, want_open: bool, dt: float) -> float:
    ...
    and letting the actuator sort it out) means the openness scalar that gets logged and
    scored is the openness the leaves are actually at.
    """
...
Measured from `g1_43dof_fixedbase.xml`: the ankle-roll link sits 0.0533 m BEHIND the
pelvis in the zero pose (hip_yaw +0.025001 x, knee -0.078273 x), and the foot's forward
contact spheres are at ankle + 0.12 with r = 0.005 -> 0.0717 m ahead of the pelvis.

Rounded UP to 0.13, because those four spheres are the contact model, not the foot: the
real foot's collision mesh is longer, and a stance that pitches forward to reach puts the
pelvis behind the toes. This is the number `TABLE_STANDOFF` has to beat.
"""
...
Measured foot reach ahead of the pelvis, from `g1_43dof_fixedbase.xml`: the ankle-roll
link sits 0.053 m BEHIND the pelvis in the zero pose (hip_yaw +0.025 x, knee -0.078 x),
and the foot's forward contact spheres are 0.12 m ahead of the ankle with r = 0.005 --
so 0.072 m ahead of the pelvis. 0.16 leaves 0.088 m for the real foot mesh (longer than
those four spheres), for a stance that pitches forward to reach, and for the fact that a
robot that walked here stopped wherever it stopped.
