...
`x` and `y` have TWO possible sources, and which one produced a given pose is not a
detail -- they differ by two orders of magnitude when the robot is not tracking its
command (TASK-231):
  ...
  * GROUND TRUTH (`parse_sim_state`, preferred). The sim publishes its whole scene
    ...
    world root pose. Nothing here estimates anything: the number on the wire is the
    number Isaac is simulating. Only a simulator can do this; on a real G1 the
    sidecar reads real odometry and none of this file runs.
  * DEAD RECKONING (`DeadReckoner`, the fallback). Integrates the velocity the bridge
    ...
    a policy that saturates -- and it NEVER self-corrects.
...
Because the two differ that much, a SILENT switch between them is the worst outcome
available here -- worse than either alone. So the source is carried on every
...
# ...and its counterpart, for a pose whose x/y came verbatim off `rt/sim_state`.
...
class GroundTruthPose(NamedTuple):
    """The sim's TRUE world root pose for one body, as `parse_sim_state` read it.
...
def parse_sim_state(payload, body: str = SIM_STATE_BODY,
    ...
    `root_pose` is indexed by environment; `num_envs` is 1 on this rig and the env
    origin is therefore zero, so element 0 is already a WORLD pose and needs no
    `env_origins` term. A multi-env run would need one, which is why the indexing is
    explicit here rather than hidden behind a `[0]` in the caller.
    ...
    poses = art.get("root_pose")
    if not isinstance(poses, (list, tuple)) or not poses:
        raise ValueError(
            f"{SIM_STATE_TOPIC} articulation {body!r} has no non-empty 'root_pose'")
    pose = poses[0]                      # num_envs = 1; see the docstring
    if not isinstance(pose, (list, tuple)) or len(pose) != 7:
        raise ValueError(
            f"{SIM_STATE_TOPIC} articulation {body!r} root_pose[0] must have 7 "
            f"components [x, y, z, qx, qy, qz, qw], got "
            f"{len(pose) if isinstance(pose, (list, tuple)) else type(pose).__name__}")
    try:
        vals = [float(v) for v in pose]
    except (TypeError, ValueError):
        raise ValueError(
            f"{SIM_STATE_TOPIC} articulation {body!r} root_pose[0] is not numeric: "
            f"{pose!r}") from None
    if not all(math.isfinite(v) for v in vals):
        ...
        raise ValueError(
            f"{SIM_STATE_TOPIC} articulation {body!r} root_pose[0] has a non-finite "
            f"component: {vals!r}")
    quat = tuple(vals[3:7])
    return GroundTruthPose(vals[0], vals[1], vals[2],
...
class OdomFrame(NamedTuple):
    ...
    `x`/`y` are either the sim's ground truth verbatim or dead reckoning shifted into
    the world frame -- `source` says which, and it is not optional garnish: the two
    disagreed by 71x on the rig that prompted TASK-231. `yaw` is measured and already
    world-absolute either way, so it carries no offset. If you are holding an OdomFrame
    you are holding world coordinates -- see "WHICH FRAME IS A GIVEN NUMBER IN" in the
    module docstring. (With no origin given and no ground truth, the two frames
    coincide, which is the old behaviour and is what makes that default byte-identical.)
    """
    x: float
    y: float
    ...
    vy_world: float
    yaw_speed: float
    # ODOM_SOURCE_GROUND_TRUTH or ODOM_SOURCE_DEAD_RECKONED, for x/y only. Last, and
    # with a default, so every existing positional construction still reads the same
    # -- and so a frame built without thinking about provenance claims the pessimistic
    # source rather than silently claiming the exact one.
...
class OdomIntegrator:
    ...
    worse than no odometry at all:
      ...
      * a ground-truth pose newer than `ground_truth_stale_after` REPLACES the
        dead-reckoned x/y outright, and one older than that is not used at all. A
        stale true pose is a frozen one, and a frozen pose reads to
        `block-executor.ts` as "the robot did not move" just as surely as a starved
    ...
    def tick(self, now: float, yaw_sample, command, ground_truth=None) -> OdomFrame | None:
        ...
        Ground truth is preferred for x/y and is used VERBATIM -- no origin, no
        filtering, no blending with the dead reckoner. Blending was considered and
        rejected: the two inputs are not two noisy measurements of the same thing, one
        is exact and the other is the command played back, and any weight on the second
        buys nothing but a number no one can reason about.
        ...
        already gates publishing, and is the one `YawTracker`/`yaw_speed` are
        differenced from -- swapping sources mid-stream would inject a step into
        `yaw_speed` for one tick every time the topic hiccuped. The ground-truth yaw is
        used as a CHECK on that heading instead (`yaw_disagreement`).
        """
        ...
        return OdomFrame(x, y, yaw_sample[0], cont,
                         vx * c - vy * s, vx * s + vy * c, yaw_speed, source)
...
def fill_odom_msg(msg, x: float, y: float, yaw: float, stamp_s: float,
    ...
    measured one. When x/y are dead reckoned it is the exact quantity whose integral
    produced them, so the two agree; when x/y are ground truth it does NOT match the
    position's derivative, and that gap is real information rather than an
    inconsistency to paper over -- it is the difference between what was asked for and
    what the base did, which is the whole subject of TASK-231. `error_code` says which
    kind of position it is sitting next to.
